...
function getCashierBranch() {
    // This is the Fairview portal: always scope to Fairview regardless of any lingering user state
    return 'SM Fairview';
}

// Normalize and match branches robustly (handles variants like "SJDM" and canonical names)
...
function initializeCashierDashboard() {
    ...
    try {
        ...
        if (branchNameDisplay) {
            branchNameDisplay.textContent = adminView ? 'All Branches' : (cashierBranch || 'SM Fairview');
        }
...
function renderInventory() {
    ...
    const cashierBranch = getCashierBranch();
    const adminView = isAdminView();
    // Update header to include Branch column when embedded in admin consolidated view
    try {
        const headerRow = document.querySelector('#inventory-list thead tr');
        if (headerRow) {
            if (adminView) headerRow.innerHTML = '<th>Item</th><th>Status</th><th>Branch</th><th>Last Updated</th>';
            else headerRow.innerHTML = '<th>Item</th><th>Status</th><th>Last Updated</th><th>Actions</th>';
        }
    } catch(_) {}
