...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // ✅ Allow cashierfairview@gmail.com or updated email from database
        $allowedEmail = 'cashierfairview@gmail.com';
        $emailAllowed = false;
        ...
        if (strtolower($email) === strtolower($allowedEmail)) {
            ...
            // Check if email exists in database for branch_id = 1 (Fairview)
            $checkStmt = $conn->prepare("SELECT branch_id FROM cashiers WHERE email = ? AND branch_id = 1");
            if ($checkStmt) {
        ...
        if ($conn && !$conn->connect_error) {
            ...
            if ($passwordValid) {
                ...
                $_SESSION['cashier_branch'] = 'fairview';
...
<head>
  ...
  <style>
    ...
    .cashier-branding h2 { margin: 0; }
    .cashier-branding p { margin: 5px 0 0 0; font-size: 14px; opacity: 0.9; }
    ...
    :root { --header-bg: #1E3A8A; --header-text: #FAF9F4; --header-accent: #60A5FA; }
    ...
    .modal { background: #fffef9; border-radius: 16px; width: min(520px, 92vw); box-shadow: 0 18px 48px rgba(0,0,0,.18); border: 1px solid rgba(0,0,0,.06); }
    .modal__header { background: linear-gradient(135deg, var(--header-bg), var(--header-accent)); color: #fff; padding: 14px 18px; display:flex; justify-content: space-between; align-items:center; }
...
<body>
  <header>
    <div class="left-header">
      ...
      <span id="portalTitle" style="font-size: 18px; font-weight: bold;">SM Fairview Cashier Portal</span>
    </div>
  ...
  <main style="display: flex; justify-content: center; align-items: center; height: calc(100vh - 80px); padding: 20px;">
    <div class="login-container">
      <div class="cashier-branding" id="cashierBranding">
        ...
        <p>Authorized cashier access only<br><strong>SM Fairview</strong></p>
      </div>
      ...
      <form class="auth-form" id="cashierLoginForm">
        ...
        <button type="submit" class="btn-main" id="loginBtn" style="background: linear-gradient(135deg, #1E3A8A, #3B82F6);">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function getCredentialsFromLocalStorage(inputEmail) {
        ...
        const creds = localStorage.getItem(CASHIER_FAIRVIEW_KEY);
        if (creds) {
          try {
